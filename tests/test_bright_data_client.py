...
def test_mcp_url_uses_configured_unlocker_zone(test_settings):
    ...
    assert configured.bright_data_mcp_unlocker_url == (
        "https://mcp.brightdata.com/mcp?token=abc&unlock=mcp_unlocker&pro=1"
    )
...
def test_mcp_url_replaces_legacy_unlock_flag(test_settings):
    ...
    assert "unlock=mcp_unlocker" in configured.bright_data_mcp_unlocker_url


def test_mcp_url_replaces_unlocker_param_with_unlock_zone(test_settings):
    configured = test_settings.model_copy(
        update={
            "bright_data_api_token": "abc",
            "bright_data_mcp_url": "https://mcp.brightdata.com/mcp?unlocker=old",
        }
    )

    assert "unlocker=old" not in configured.bright_data_mcp_unlocker_url
    assert "unlock=mcp_unlocker" in configured.bright_data_mcp_unlocker_url
...
def test_unlocker_url_strips_shell_quotes(test_settings):
    ...
    assert configured.bright_data_mcp_unlocker_url == (
        "https://mcp.brightdata.com/mcp?token=abc&pro=1&unlock=mcp_unlocker"
    )
